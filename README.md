...
- **Docker** (Required by Supabase CLI to run local services)
- **Node.js** & **npm** (To run `npm install` for the Supabase CLI wrapper, though `supabase` CLI can be installed directly)
- **Git**
...
Currently, the core database and edge functions rely heavily on environment variables auto-injected by the Supabase CLI (like `SUPABASE_URL` and `SUPABASE_SERVICE_ROLE_KEY`). 

However, `supabase/config.toml` expects an `OPENAI_API_KEY` for Supabase Studio AI features.
*(Recommended: Create a `.env` file at the root of the project to store optional secrets.)*

```env
# .env.example
OPENAI_API_KEY=your_openai_key_here # Optional: For Supabase Studio AI assistance
```
...
2. **Install dependencies:**
   ...
   npm install
   ```
   *(This installs the local `supabase` CLI defined in `package.json`.)*
...
3. **Start local Supabase:**
   ...
   npx supabase start
   ```
   *This command pulls the necessary Docker images, starts the database, applies migrations in `supabase/migrations/`, and runs `supabase/seed.sql`.*
...
- **Access Local Studio:** After running `supabase start`, open [http://127.0.0.1:56323](http://127.0.0.1:56323) to view the database UI.
- **Reset the Database:** 
...
- **Apply New Migrations:** 
  If you pull new migrations from `main` while your local Supabase is running, apply them with:
  ```bash
  npx supabase db reset
  ```
  *(Or `npx supabase db push` depending on your environment, but reset ensures seeds are re-applied locally).*
...
- **Testing:** You can invoke them locally using tools like `curl` or Postman. For example:
  ```bash
  ...
  ```
  *(Note: You can find your anon key in the output of `npx supabase status`)*

## Verification / Smoke Test

To verify the setup was successful:
1. Run `npx supabase status` to ensure all services are running.
...
3. Check the Edge Functions output in the terminal where you ran `supabase functions serve`. It should say `Functions are ready`.
...
| `npx supabase stop` | Stops local Supabase stack |
| `npx supabase status` | Shows local endpoints and keys |
| `npx supabase db reset` | Recreates database, runs migrations and seeds |
...
- **Ports already in use:** If ports `56321`-`56327` are taken, check `supabase/config.toml` to customize them or stop other local projects.
- **Missing Edge Function Types:** If you open Deno files and see missing imports, make sure your IDE has a Deno extension installed and initialized for the `supabase/functions` directory.
